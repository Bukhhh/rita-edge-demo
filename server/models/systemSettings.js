...
const defaultRitaAgents = [
  {
    id: "rita-report-agent",
    name: "RITA - Report Agent",
    description:
      "Create structured reports, executive summaries, and PDF-ready outputs.",
    enabled: true,
    icon: "RA",
    color: "#2563eb",
    version: "1.0.0",
  ...
  {
    id: "rita-graph-agent",
    name: "RITA - Graph Agent",
    description:
      "Create charts, visual insights, and graph-based report outputs.",
    enabled: true,
    icon: "GA",
    color: "#16a34a",
    version: "1.0.0",
...
function normalizeRitaPdfTemplate(update = {}) {
  const incoming =
  ...
  const allowedChartSizes = ["medium", "large", "full"];
  const colorPattern = /^#[0-9a-fA-F]{6}$/;

  return {
    logo_position: allowedLogoPositions.includes(incoming.logo_position)
      ? incoming.logo_position
      : defaultRitaPdfTemplate.logo_position,
    show_logo:
      incoming.show_logo === undefined
        ? defaultRitaPdfTemplate.show_logo
        : incoming.show_logo === true || incoming.show_logo === "true",
    show_page_numbers:
      incoming.show_page_numbers === undefined
        ? defaultRitaPdfTemplate.show_page_numbers
        : incoming.show_page_numbers === true ||
          incoming.show_page_numbers === "true",
    footer_text:
      typeof incoming.footer_text === "string"
        ? incoming.footer_text.slice(0, 120)
        : defaultRitaPdfTemplate.footer_text,
    primary_color: colorPattern.test(incoming.primary_color)
      ? incoming.primary_color
      : defaultRitaPdfTemplate.primary_color,
    accent_color: colorPattern.test(incoming.accent_color)
      ? incoming.accent_color
      : defaultRitaPdfTemplate.accent_color,
    chart_size: allowedChartSizes.includes(incoming.chart_size)
      ? incoming.chart_size
      : defaultRitaPdfTemplate.chart_size,
  };
}

function normalizeRitaCapabilities(update = {}) {
  const incoming =
    typeof update === "string" ? safeJsonParse(update, {}) : update ?? {};

  return Object.keys(defaultRitaCapabilities).reduce((acc, key) => {
    acc[key] =
      incoming[key] === undefined
        ? defaultRitaCapabilities[key]
        : incoming[key] === true || incoming[key] === "true";
    return acc;
  }, {});
}

function normalizeRitaProviderControls(update = {}) {
  const incoming =
    typeof update === "string" ? safeJsonParse(update, {}) : update ?? {};

  return Object.keys(defaultRitaProviderControls).reduce((acc, key) => {
    acc[key] =
      incoming[key] === undefined
        ? defaultRitaProviderControls[key]
        : incoming[key] === true || incoming[key] === "true";
    return acc;
  }, {});
}
...
function normalizeRitaAgents(update = []) {
  ...
  return defaultRitaAgents.map((agent) => {
    const incomingAgent = incomingById[agent.id] || {};
    return {
      ...agent,
      name:
        typeof incomingAgent.name === "string" && incomingAgent.name.trim()
          ? incomingAgent.name.trim().slice(0, 80)
          : agent.name,
      description:
        typeof incomingAgent.description === "string" &&
        incomingAgent.description.trim()
          ? incomingAgent.description.trim().slice(0, 220)
          : agent.description,
      enabled:
        incomingAgent.enabled === undefined
          ? agent.enabled
          : incomingAgent.enabled === true || incomingAgent.enabled === "true",
      color: /^#[0-9a-fA-F]{6}$/.test(incomingAgent.color)
        ? incomingAgent.color
        : agent.color,
      version:
      ...
      instructions:
        ...
        incomingAgent.instructions.trim()
          ? incomingAgent.instructions.trim().slice(0, 1500)
          : agent.instructions,
    };
  });
}
...
const SystemSettings = {
  ...
  validations: {
    ...
    rita_capabilities: (update) => {
      try {
        return JSON.stringify(normalizeRitaCapabilities(update));
      } catch (e) {
        console.error(`Could not validate RITA capabilities.`, e.message);
        return JSON.stringify(defaultRitaCapabilities);
      }
    },
    rita_agents: (update) => {
      try {
        return JSON.stringify(normalizeRitaAgents(update));
      } catch (e) {
        console.error(`Could not validate RITA agents.`, e.message);
        return JSON.stringify(defaultRitaAgents);
      }
    },
    rita_pdf_template: (update) => {
      try {
        return JSON.stringify(normalizeRitaPdfTemplate(update));
      } catch (e) {
        console.error(`Could not validate RITA PDF template.`, e.message);
        return JSON.stringify(defaultRitaPdfTemplate);
      }
    },
    rita_provider_controls: (update) => {
      try {
        return JSON.stringify(normalizeRitaProviderControls(update));
      } catch (e) {
        console.error(`Could not validate RITA provider controls.`, e.message);
        return JSON.stringify(defaultRitaProviderControls);
      }
    },
  ...
  currentSettings: async function () {
    ...
    return {
      ...
      RitaCapabilities: await this.ritaCapabilities(),
      RitaAgents: await this.ritaAgents(),
      RitaPdfTemplate: await this.ritaPdfTemplate(),
      RitaProviderControls: await this.ritaProviderControls(),
  ...
  ritaCapabilities: async function () {
    const setting = await SystemSettings.get({ label: "rita_capabilities" });
    return normalizeRitaCapabilities(setting?.value);
  },
  ritaAgents: async function () {
    const setting = await SystemSettings.get({ label: "rita_agents" });
    return normalizeRitaAgents(setting?.value);
  },
  ritaAgentById: async function (id = null) {
    if (!id) return null;
    const agents = await SystemSettings.ritaAgents();
    return agents.find((agent) => agent.id === id && agent.enabled) || null;
  },
  ritaPdfTemplate: async function () {
    const setting = await SystemSettings.get({ label: "rita_pdf_template" });
    return normalizeRitaPdfTemplate(setting?.value);
  },
  ritaProviderControls: async function () {
    const setting = await SystemSettings.get({
      label: "rita_provider_controls",
    });
    return normalizeRitaProviderControls(setting?.value);
  },
